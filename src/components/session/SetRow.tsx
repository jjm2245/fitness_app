...
export function SetRow({
        ...
        <div className={styles.setEditRow} style={isDrop ? { paddingLeft: 22 } : undefined}>
          ...
          <span className={styles.seg}>
            {EFFORT_OPTIONS.map((o) => (
              <button key={o.value} type="button" onClick={() => setEffort(o.value)} className={effort === o.value ? styles.segActive : styles.segBtn}>
                {o.label}
              </button>
            ))}
          </span>
  ...
  const loadText =
    set.builtinOffset != null && set.builtinOffset !== 0 && set.loadEntered != null
      ? `${set.loadEntered} + ${set.builtinOffset} = ${set.load}`
      : `${set.load}`;
