...
- **Migrations:** 37 on disk (latest `0036_equipment_comparability`); `EXPECTED_MIGRATIONS = 37`.
- **Tests:** ~400 cases across 30 `*.test.ts` files.
...
| Ratio-estimate overlay (G4-skipped) | First differing-ratio cable pair sharing an exercise in `set_logs` → build the ratio-estimate overlay branch and complete checklist #5's estimated-point half (currently verified only as decision-invariance). |
...
- **Open UX questions from Stats v1 — recorded, not resolved, awaiting the
  owner's phone review:** (a) the hub PR counter uses the CALENDAR month, so it
  reads "0 PRs in August" on Aug 3 with a July full of PRs behind it; candidate
  fix is a rolling 30 days; decide after it's been seen on the phone. (b) the
  combined timeline's machine-switch delta compares against the same machine's
  previous session, which can skip the visually adjacent row — correct by rule,
  possibly surprising to read; accepted risk pending screenshots.
...
- **Hub** (`/stats`): tiles lit in place via the extended `LockedTile` —
  Exercises live with a computed subline (tracked · PRs this month · last
  trained), Muscles tagged `next`, Body/Recovery/Nutrition still locked.
- **Index** (`/stats/exercises`): one row per exercise present in `set_logs`
  (~28), never the manage payload. Arc subline in the delta grammar (bright
  only for progress), best figure mono right-aligned, PR chip only when the
  latest session set that machine's best. Reps lanes read `last N reps`,
  never "best".
- **Exercise screen** (`/stats/exercises/[id]`): List = one combined
  chronological timeline (sessions keyed on `workout_logs.id` — two sessions
  on one date stay two rows), machine tag per row, six-state delta grammar
  against the SAME machine's previous session. Chart = per-machine sections,
  inline SVG, PR points in `--accent-2`; a machine with one session keeps its
  list row (no one-dot charts). View choice persisted in `localStorage`.
- **Routes**: `GET /api/stats/exercises`, `GET /api/stats/exercises/[id]`,
  `POST /api/stats/comparability`. All figures/deltas/PRs computed on read
  from `set_logs` via `src/lib/statsShape.ts` + `src/lib/prs.ts` (`markPrs` —
  no parallel strictly-greater in SQL).
- **Comparability**: engine in `src/lib/comparability.ts` (pure; deliberately
  NOT `src/core/*`, which stays equipment-blind). Suggestions computed on
  demand at request time — never stored, no cache — so **filling a unit's
  specs activates them with zero code changes** (transition test in
  `comparability.test.ts`). Decisions-only storage in `equipment_comparability`
  (0036): pair-ordered, kind+status, basis snapshotted at decision time;
  flip updates in place. **Dormant in prod, and exactly why:** the only
  two-machine exercise pairs VSL18 with LifeFitnessShoulder, whose specs are
  all NULL and pulley `unknown` — both-unknown ≠ both-known, so
  `suggestSameSetup` correctly refuses. LFS is the deliberate NULL-spec test
  asset until the owner measures it at the gym.
- **G4 branches skipped, with triggers**: the ratio-estimate overlay (trigger:
  a differing-ratio plain-cable pair sharing an exercise) and the same-setup
  shared-axis merge rendering (trigger: a confirmed spec-identical pair).
  The suggestion card + decision line are data-driven, not stubs.
- **`core/volume.ts` is still unimported by Stats** — v1 never touches it, so
